...
fn test_non_mod_files_ignored() {
    ...
    assert_eq!(
        files.len(),
        2,
        "Should detect both standard and generic archive files"
    );
    assert!(files
        .iter()
        .any(|f| f.file_name == "ValidMod-1000-2000-1-0-1234567890.7z"));
...
fn test_parse_real_files() {
    use wabbajack_library_cleaner::core::{is_wabbajack_file, parse_mod_filename};

    let files = [
        (
            "1) Point That Somewhere Else - Main File-73938-2-22-2-1766239208.zip",
            true,
            Some("73938"),
        ),
        ("BHYSYS-71112-13-02-1766329383.rar", true, Some("71112")),
        (
            "Pip-Boy UI Tweaks-85343-5-0-1-1766262984.zip",
            true,
            Some("85343"),
        ),
    ...
    for (filename, should_be_wabbajack, expected_mod_id) in files {
        ...
        println!(
            "  is_wabbajack_file: {} (expected: {})",
            is_wb, should_be_wabbajack
        );

        if let Some(parsed) = parse_mod_filename(filename) {
            println!(
                "  mod_id: {} (expected: {:?})",
                parsed.mod_id, expected_mod_id
            );
...
fn test_parse_file_id() {
    ...
    let files = [
        (
            ...
            "73938",
            None,
        ), // No file_id in Nexus format
        (
            "SkyUI_5_2SE-12604-5-2SE-52344-1615410779.7z",
            "12604",
            Some("52344"),
        ), // Test format with file_id
    ];
    ...
    for (filename, expected_mod_id, expected_file_id) in files {
        if let Some(parsed) = parse_mod_filename(filename) {
            ...
            println!(
                "  file_id: {:?} (expected: {:?})",
                parsed.file_id, expected_file_id
            );
        }
    }
}
...
fn test_real_modlist_parsing() {
    ...
    assert!(
        modlist_info.mod_count >= 50,
        "Real modlist should have at least 50 mods"
    );
    assert!(
        !modlist_info.used_mod_keys.is_empty(),
        "Should have detected ModIDs"
    );
}
...
fn test_simulation_with_real_modlist() {
    ...
    let real_files: Vec<String> = modlist_info
        .used_file_names
        .iter()
        .take(5)
        .cloned()
    ...
    assert_eq!(
        all_files.len(),
        total_expected,
        "Should have detected all created files"
    );

    // Check Orphans
    let orphans_found = orphan_result
        .orphaned_mods
        .iter()
    ...
    assert!(
        real_files_found >= 5,
        "Should detect at least the 5 real files as used"
    );

    // Check Old Versions
    let duplicate_groups = old_ver_result.duplicates.len();
    assert_eq!(
        duplicate_groups, old_versions_created,
        "Should detect correct number of duplicate groups"
    );
