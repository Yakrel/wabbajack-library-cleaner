...
use crate::core::parser::{
    extract_part_indicator, is_full_or_main_file, is_wabbajack_file, 
    normalize_mod_name, parse_mod_filename,
};
use crate::core::types::{
    LibraryStats, ModFile, ModGroup, ModlistInfo, OldVersionScanResult, 
    OrphanedMod, ScanResult,
};
...
pub fn get_game_folders(base_dir: &Path) -> Result<Vec<std::path::PathBuf>> {
    ...
    if has_mod_files {
        log::info!("Selected directory contains mod files, including it: {:?}", base_dir);
...
pub fn find_wabbajack_files(base_dir: &Path) -> Result<Vec<std::path::PathBuf>> {
    ...
    for entry in entries {
        ...
        let name = entry.file_name();
        if name.to_string_lossy().to_lowercase().ends_with(".wabbajack") {
...
pub fn detect_orphaned_mods(
    ...
    log::info!("Total unique ModIDs in active modlists: {}", used_mod_ids.len());
    log::info!("Total unique ModID+FileID pairs: {}", used_mod_file_ids.len());
    
    let mut used_mods = Vec::new();
    let mut orphaned_mods = Vec::new();
    ...
    for mod_file in mod_files {
        ...
        if is_used {
            used_size += mod_file.size;
            used_mods.push(mod_file.clone());
        } else {
            orphaned_size += mod_file.size;
            orphaned_mods.push(OrphanedMod { file: mod_file.clone() });
        }
    }
    
    log::info!("Classification complete: {} used, {} orphaned", used_mods.len(), orphaned_mods.len());
...
fn has_conflicting_descriptors(filename1: &str, filename2: &str) -> bool {
    ...
    let descriptors1: Vec<_> = all_descriptors.iter()
        .filter(|d| lower1.contains(*d))
        .collect();
    let descriptors2: Vec<_> = all_descriptors.iter()
        .filter(|d| lower2.contains(*d))
        .collect();
    
    // If one file has descriptors but the other doesn't
    if (descriptors1.is_empty() != descriptors2.is_empty()) && (!descriptors1.is_empty() || !descriptors2.is_empty()) {
        return true;
    }
        ...
        for j in i + 1..group.files.len() {
            ...
            if file1.version == file2.version {
                ...
                if size_ratio > 10.0 || size_ratio < 0.1 {
                    log::warn!("Group {}: Same version '{}' but size diff >10x", 
                        group.mod_key, file1.version);
                    return true;
                }
                ...
                if let (Ok(ts1), Ok(ts2)) = (
                    ...
                    if time_diff < 3600 {
                        log::warn!("Group {}: Same version '{}' uploaded within 1 hour",
                            group.mod_key, file1.version);
                        return true;
                    }
                }
            ...
            if has_conflicting_descriptors(&file1.file_name, &file2.file_name) {
                log::warn!("Group {}: Files have conflicting descriptors", group.mod_key);
                return true;
            }
        }
    }
...
pub fn scan_folder_for_duplicates(folder_path: &Path) -> Result<OldVersionScanResult> {
    ...
    for (_, mut group) in mod_groups {
        ...
        if unique_timestamps.len() <= 1 {
            log::info!("Skipped group {}: all files have same timestamp", group.mod_key);
            continue;
        }
        ...
        if has_suspicious_version_pattern(&group) {
            log::warn!("Skipped group {}: suspicious version pattern", group.mod_key);
            continue;
        }
        
        // Check for patch/main file combinations
        let has_patch = group.files.iter().any(|f| f.is_patch);
        let has_main = group.files.iter().any(|f| is_full_or_main_file(&f.file_name));
        
        if has_patch && has_main {
            log::warn!("Skipped group {}: contains both PATCH and MAIN files", group.mod_key);
            continue;
        }
        ...
        if newest.is_patch && group.files.len() > 1 {
            for i in 0..group.files.len() - 1 {
                ...
                if size_ratio < 0.1 {
                    log::warn!("Skipped group {}: newest file is likely a patch", group.mod_key);
                    skip_patch = true;
                    break;
                }
            }
        ...
        group.space_to_free = group.files[..group.newest_idx]
            .iter()
            .map(|f| f.size)
            .sum();
...
pub fn calculate_library_stats(game_folders: &[std::path::PathBuf]) -> LibraryStats {
    ...
    for folder in game_folders {
        ...
        stats.by_game.push((game_name, game_files, game_size));
        stats.total_files += game_files;
        stats.total_size += game_size;
    }
