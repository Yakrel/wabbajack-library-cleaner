...
pub fn parse_mod_filename(filename: &str) -> Option<ModFile> {
    // Check extension
    let ext = ARCHIVE_EXTENSIONS.iter()
...
pub fn parse_wabbajack_file(file_path: &Path) -> Result<ModlistInfo> {
    ...
    let mut archive = ZipArchive::new(file)
        .with_context(|| "Failed to read wabbajack file as ZIP")?;
    ...
    let mut modlist_content = String::new();
    {
        ...
        modlist_file.read_to_string(&mut modlist_content)
            .with_context(|| "Failed to read modlist file")?;
    }
    
    let modlist: Modlist = serde_json::from_str(&modlist_content)
    ...
    log::info!("Parsed modlist '{}': {} archives, {} unique ModIDs, {} ModID+FileID pairs",
        modlist.name, modlist.archives.len(), used_mod_keys.len(), used_mod_file_ids.len());
