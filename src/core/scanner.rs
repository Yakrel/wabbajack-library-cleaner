...
pub fn get_all_mod_files(game_folders: &[std::path::PathBuf]) -> Result<Vec<ModFile>> {
    ...
    let all_files: Vec<ModFile> = game_folders.par_iter()
        .flat_map(|folder| {
...
pub fn detect_orphaned_mods(
    ...
    let (used_mods, orphaned_mods): (Vec<ModFile>, Vec<OrphanedMod>) = mod_files.par_iter()
        .partition_map(|mod_file| {
            ...
            if is_used {
                ...
                rayon::iter::Either::Right(OrphanedMod { file: mod_file.clone() })
            }
        });
...
pub fn calculate_library_stats(game_folders: &[std::path::PathBuf]) -> LibraryStats {
    let results: Vec<(String, usize, u64)> = game_folders.par_iter()
        .map(|folder| {
            let entries = match fs::read_dir(folder) {
                Ok(e) => e,
