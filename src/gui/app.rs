...
impl WabbajackCleanerApp {
    ...
    fn log(&mut self, level: LogLevel, msg: &str) {
        ...
        self.log_messages.push((format!("[{}] {}", time, msg), level));
    ...
    fn select_downloads_dir(&mut self) {
            ...
            .pick_folder()
        {
            ...
            thread::spawn(move || match get_game_folders(&path) {
                Ok(folders) => { tx.send(AsyncMessage::GameFoldersFound(folders)).ok(); }
                Err(e) => { tx.send(AsyncMessage::Error(e.to_string())).ok(); }
            });
        }
    }
    ...
    fn run_orphaned_scan(&mut self, delete: bool) {
        let selected: Vec<ModlistInfo> = self.modlists.iter().enumerate()
        ...
        self.is_loading = true;
        self.current_operation = if delete { "Cleaning orphaned mods..." } else { "Scanning for orphaned mods..." }.to_string();
        ...
        while let Ok(msg) = self.rx.try_recv() {
            match msg {
                AsyncMessage::ModlistsParsed(list) => {
                    ...
                    self.progress = None;
                    if self.downloads_dir.is_some() { self.run_analysis(); }
                }
                AsyncMessage::GameFoldersFound(folders) => {
                    self.log(LogLevel::Info, &format!("Found {} game folders", folders.len()));
                    self.game_folders = folders;
                    self.progress = None;
                    if self.wabbajack_dir.is_some() { self.run_analysis(); }
                }
                ...
                AsyncMessage::OrphanedScanComplete(res) => {
                    self.log(LogLevel::Info, &format!("Found {} orphaned files ({})", res.orphaned_mods.len(), format_size(res.orphaned_size)));
                    self.orphaned_result = Some(res);
                    self.is_loading = false;
                    self.progress = None;
                }
                AsyncMessage::OldVersionScanComplete(res) => {
                    self.log(LogLevel::Info, &format!("Found {} old versions ({})", res.total_files, format_size(res.total_space)));
                    self.old_version_result = Some(res);
                    self.is_loading = false;
                    self.progress = None;
                }
                AsyncMessage::DeletionComplete(res) => {
                    self.log(LogLevel::Info, &format!("Cleaned {} files, freed {}", res.deleted_count, format_size(res.space_freed)));
                    self.is_loading = false;
                    self.progress = None;
                    self.run_analysis();
                }
...
impl eframe::App for WabbajackCleanerApp {
    fn update(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::TopBottomPanel::top("header")
            .exact_height(50.0)
            .frame(egui::Frame::none().fill(COLOR_BG_HEADER).inner_margin(egui::vec2(16.0, 12.0)))
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Wabbajack Library Cleaner").size(18.0).strong().color(COLOR_TEXT_PRIMARY));
                    ui.label(RichText::new(format!("v{}", APP_VERSION)).size(12.0).color(COLOR_TEXT_MUTED));
        ...
        egui::TopBottomPanel::bottom("log_panel")
            .resizable(false)
            .exact_height(120.0)
            .frame(egui::Frame::none().fill(COLOR_BG_HEADER).inner_margin(egui::vec2(12.0, 8.0)))
            .show(ctx, |ui| {
                ui.set_width(ui.available_width());
                ui.horizontal(|ui| {
                    if self.is_loading {
                        ui.spinner();
                        ui.label(RichText::new(&self.current_operation).color(COLOR_TEXT_SECONDARY));
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui.small_button("Copy Log").clicked() {
                            let log_text: String = self.log_messages.iter()
...
impl WabbajackCleanerApp {
    fn section_frame(ui: &mut egui::Ui, title: &str, add_contents: impl FnOnce(&mut egui::Ui)) {
        egui::Frame::none()
            ...
            .show(ui, |ui| {
                ui.set_width(ui.available_width());
                ui.label(RichText::new(title).size(13.0).strong().color(COLOR_TEXT_PRIMARY));
                ui.add_space(8.0);
                add_contents(ui);
            });
    }
    ...
    fn render_paths_section(&mut self, ui: &mut egui::Ui) {
        Self::section_frame(ui, "Step 1: Select Folders", |ui| {
            ui.columns(2, |cols| {
                ...
                cols[0].label(RichText::new("Folder containing Wabbajack.exe").size(11.0).color(COLOR_TEXT_MUTED));
                ...
                cols[0].horizontal(|ui| {
                    ...
                    if let Some(p) = &self.wabbajack_dir {
                        ui.label(RichText::new(p.file_name().unwrap_or_default().to_string_lossy()).color(COLOR_SUCCESS));
                    } else {
                ...
                cols[1].label(RichText::new("Wabbajack mod downloads location").size(11.0).color(COLOR_TEXT_MUTED));
            ...
            if let Some(stats) = &self.stats {
                ...
                ui.add_space(4.0);
                ui.horizontal(|ui| {
                    ui.label(RichText::new(format!("{} files", stats.total_files)).size(12.0).color(COLOR_TEXT_SECONDARY));
                    ui.label(RichText::new(" | ").color(COLOR_TEXT_MUTED));
                    ui.label(RichText::new(format_size(stats.total_size)).size(12.0).color(COLOR_ACCENT));
                    ui.label(RichText::new(" | ").color(COLOR_TEXT_MUTED));
                    ui.label(RichText::new(format!("{} game folders", self.game_folders.len())).size(12.0).color(COLOR_TEXT_SECONDARY));
                });
            }
        });
    }
    ...
    fn render_modlist_section(&mut self, ui: &mut egui::Ui) {
        Self::section_frame(ui, "Step 2: Select Modlists to Protect", |ui| {
            if self.modlists.is_empty() {
                ...
                ui.horizontal(|ui| {
                    ui.label(RichText::new(format!("{}/{} selected", self.selected_modlist_count(), self.modlists.len())).size(12.0).color(COLOR_TEXT_SECONDARY));
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                ...
                egui::ScrollArea::vertical().max_height(100.0).show(ui, |ui| {
                    for (i, ml) in self.modlists.iter().enumerate() {
                        ...
                        let color = if checked { COLOR_TEXT_PRIMARY } else { COLOR_TEXT_MUTED };
                        if ui.checkbox(&mut new_checked, RichText::new(format!("{} ({} mods)", ml.name, ml.mod_count)).color(color)).changed() {
                            if let Some(sel) = self.modlist_selected.get_mut(i) { *sel = new_checked; }
                        }
                    }
                });
            }
    ...
    fn render_actions_section(&mut self, ui: &mut egui::Ui) {
        Self::section_frame(ui, "Step 3: Cleanup Actions", |ui| {
            ...
            ui.columns(2, |cols| {
                // Orphaned Mods
                cols[0].label(RichText::new("Orphaned Mods").strong().color(COLOR_TEXT_PRIMARY));
                cols[0].label(RichText::new("Mods not used by selected modlists").size(11.0).color(COLOR_TEXT_MUTED));
                cols[0].add_space(4.0);
                cols[0].horizontal(|ui| {
                    if ui.add_enabled(ready, egui::Button::new("Analyze")).clicked() {
                        self.run_orphaned_scan(false);
                    }
                    if ui.add_enabled(ready, egui::Button::new(RichText::new("Clean").color(COLOR_TEXT_PRIMARY)).fill(COLOR_DANGER)).clicked() {
                        self.run_orphaned_scan(true);
                    }
                });

                // Old Versions
                cols[1].label(RichText::new("Old Versions").strong().color(COLOR_TEXT_PRIMARY));
                cols[1].label(RichText::new("Duplicate mods with newer versions").size(11.0).color(COLOR_TEXT_MUTED));
                cols[1].add_space(4.0);
                cols[1].horizontal(|ui| {
                    if ui.add_enabled(ready, egui::Button::new("Analyze")).clicked() {
                        self.run_old_version_scan(false);
                    }
                    if ui.add_enabled(ready, egui::Button::new(RichText::new("Clean").color(COLOR_TEXT_PRIMARY)).fill(COLOR_WARNING)).clicked() {
    ...
    fn render_results_section(&mut self, ui: &mut egui::Ui) {
        if self.orphaned_result.is_none() && self.old_version_result.is_none() { return; }

        Self::section_frame(ui, "Results", |ui| {
            if let Some(res) = &self.orphaned_result {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Orphaned Mods:").strong().color(COLOR_TEXT_PRIMARY));
                    ui.label(RichText::new(format!("{} files", res.orphaned_mods.len())).color(COLOR_TEXT_SECONDARY));
                    ui.label(RichText::new(format_size(res.orphaned_size)).color(COLOR_DANGER));
                });
                egui::ScrollArea::vertical().max_height(120.0).id_salt("orphaned").show(ui, |ui| {
                    for m in &res.orphaned_mods {
                        ui.horizontal(|ui| {
                            ui.label(RichText::new(&m.file.file_name).size(11.0).color(COLOR_TEXT_PRIMARY));
                            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                                ui.label(RichText::new(format_size(m.file.size)).size(11.0).color(COLOR_TEXT_MUTED));
                            });
                        });
                    }
                });
            ...
            if let Some(res) = &self.old_version_result {
                ui.horizontal(|ui| {
                    ui.label(RichText::new("Old Versions:").strong().color(COLOR_TEXT_PRIMARY));
                    ui.label(RichText::new(format!("{} files", res.total_files)).color(COLOR_TEXT_SECONDARY));
                    ui.label(RichText::new(format_size(res.total_space)).color(COLOR_WARNING));
                });
                egui::ScrollArea::vertical().max_height(150.0).id_salt("oldver").show(ui, |ui| {
                    for group in &res.duplicates {
                        ui.label(RichText::new(&group.mod_key).size(11.0).strong().color(COLOR_ACCENT));
                        for (i, f) in group.files.iter().enumerate() {
                            let is_keep = i == group.newest_idx;
                            let (status, color) = if is_keep { ("KEEP", COLOR_SUCCESS) } else { ("DELETE", COLOR_DANGER) };
                            ui.horizontal(|ui| {
                                ui.label(RichText::new(format!("  {} - {}", status, f.file_name)).size(11.0).color(color));
                                ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
    ...
    fn render_modals(&mut self, ctx: &egui::Context) {
        if self.modal == Modal::About {
            egui::Window::new("About")
                .collapsible(false).resizable(false).default_width(800.0)
                ...
                .show(ctx, |ui| {
                    ui.columns(2, |cols| {
                        ...
                        cols[0].vertical_centered(|ui| {
                            ui.add_space(20.0);
                            ui.label(RichText::new("Wabbajack Library Cleaner").size(24.0).strong().color(COLOR_TEXT_PRIMARY));
                            ui.label(RichText::new(format!("Version {}", APP_VERSION)).size(14.0).color(COLOR_TEXT_SECONDARY));
                            ui.add_space(20.0);
                            ui.label(RichText::new("Clean up your Wabbajack downloads folder").size(14.0).color(COLOR_TEXT_SECONDARY));
                            ui.label(RichText::new("safely and efficiently.").size(14.0).color(COLOR_TEXT_SECONDARY));
                            ui.add_space(30.0);
                            ui.label(RichText::new("Created by Berkay Yetgin").color(COLOR_TEXT_MUTED));
                            ui.add_space(8.0);
                            ui.hyperlink_to("GitHub Repository", "https://github.com/Yakrel/wabbajack-library-cleaner");
                            ui.add_space(8.0);
                            ui.label(RichText::new("License: GPL-3.0").size(11.0).color(COLOR_TEXT_MUTED));
                        });

                        // Right Column: Changelog
                        cols[1].vertical(|ui| {
                            ui.label(RichText::new("Changelog").strong().size(16.0).color(COLOR_TEXT_PRIMARY));
                            ui.add_space(8.0);
                            egui::ScrollArea::vertical().max_height(300.0).show(ui, |ui| {
                                ...
                                for line in changelog.lines() {
                                    if let Some(c) = line.strip_prefix("## ") {
                                        ...
                                        ui.label(RichText::new(c).strong().size(12.0).color(COLOR_TEXT_PRIMARY));
                                    } else if let Some(c) = line.strip_prefix("- ") {
                                        ui.label(RichText::new(format!("• {}", c)).size(12.0).color(COLOR_TEXT_SECONDARY));
                                    }
                                }
                            });
                        });
                    ...
                    ui.vertical_centered(|ui| {
                        if ui.button(RichText::new("Close").size(14.0)).clicked() { self.modal = Modal::None; }
                    });
                });
        }
        ...
        if self.modal == Modal::FolderSelect {
            egui::Window::new("Select Game Folder")
                .collapsible(false).resizable(false).default_width(350.0)
                .anchor(egui::Align2::CENTER_CENTER, [0.0, 0.0])
                .show(ctx, |ui| {
                    ...
                    ui.add_space(8.0);
                    ui.horizontal(|ui| {
                        if ui.add_enabled(self.selected_game_folder.is_some(), egui::Button::new("Start Scan").fill(COLOR_ACCENT)).clicked() {
...
fn scan_wabbajack_dir(path: PathBuf, tx: Sender<AsyncMessage>) {
    tx.send(AsyncMessage::Progress("Scanning...".to_string(), None)).ok();
    let entries = match std::fs::read_dir(&path) {
        Ok(e) => e,
        Err(e) => { tx.send(AsyncMessage::Error(e.to_string())).ok(); return; }
    };
        ...
        if modlists_path.exists() {
            if let Ok(files) = find_wabbajack_files(&modlists_path) {
                for wbfile in files {
                    ...
                    if modlist_map.get(&key).map(|(_, v)| &version_name > v).unwrap_or(true) {
    ...
    if modlist_map.is_empty() {
        tx.send(AsyncMessage::Error("No modlists found.".to_string())).ok();
        return;
    }
    ...
    for (i, (_, (p, _))) in modlist_map.into_iter().enumerate() {
        tx.send(AsyncMessage::Progress("Parsing modlists...".to_string(), Some((i + 1, total)))).ok();
        if let Ok(info) = parse_wabbajack_file(&p) { modlists.push(info); }
    }
    tx.send(AsyncMessage::ModlistsParsed(modlists)).ok();
}

fn scan_orphaned_mods_async(path: PathBuf, modlists: Vec<ModlistInfo>, delete: bool, backup: Option<PathBuf>, tx: Sender<AsyncMessage>) {
    tx.send(AsyncMessage::Progress("Indexing files...".to_string(), None)).ok();
    ...
    tx.send(AsyncMessage::Progress(format!("Analyzing {} files...", files.len()), None)).ok();
    let result = detect_orphaned_mods(&files, &modlists);
    if delete && !result.orphaned_mods.is_empty() {
        tx.send(AsyncMessage::Progress("Cleaning...".to_string(), Some((0, result.orphaned_mods.len())))).ok();
...
fn scan_old_versions_async(path: PathBuf, delete: bool, backup: Option<PathBuf>, tx: Sender<AsyncMessage>) {
    tx.send(AsyncMessage::Progress("Scanning...".to_string(), None)).ok();
    ...
    if delete && !result.duplicates.is_empty() {
        tx.send(AsyncMessage::Progress("Cleaning...".to_string(), Some((0, result.total_files)))).ok();
